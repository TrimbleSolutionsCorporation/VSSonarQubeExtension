...
let GetSettings(newConf : ISonarConfiguration, project : Resource, httpconnector : IHttpSonarConnector) =
    ...
    let CreateSetting(i:SettingsValues.Setting) = 
        ...
        let AddFieldValue(fieldValue:SettingsValues.FieldValue) = 
            ...
            let textValue = fieldValue.JsonValue.ToString().TrimStart('{').TrimEnd('}').Trim()
            let elements = textValue.Split(',')
            for elem in elements  do
                let key = elem.Split(':').[0].Replace("\"", "")
                let value = elem.Split(':').[1].Replace("\"", "")
                valueData.Values.Add(key.Trim(), value.Trim())

            setting.FieldValues.Add(valueData)

        setting.key <- i.Key
        setting.Inherited <- i.Inherited
