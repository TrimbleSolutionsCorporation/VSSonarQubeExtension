    ...
    public class JiraPlugin : IIssueTrackerPlugin
    {
        ...
        private string GatherNotes(IList<Issue> issues, Resource project, out string summaryOut)
        {
            ...
            foreach (var issue in issues)
            {
                ...
                if (this.userConf.SonarVersion < 6.3)
                {
                    notes.AppendLine(url + "[See in SonarQube|" + this.userConf.Hostname.TrimEnd('/') + "/issues/search#issues=" + issue.Key + "]");
                }
                else
                { 
                    notes.AppendLine(url + "[Open Issue|" + this.userConf.Hostname.TrimEnd('/') + "/issues/search#issues=" + issue.Key + "] [Open File|" + this.userConf.Hostname.TrimEnd('/') + "/component?id=" + issue.Component + "&line=" + issue.Line + "]");
                }
        ...
        private string GatherNotesInPlainText(IList<Issue> issues, Resource project)
        {
            ...
            {
                var compElelms = issue.Component.Split(':');
                var data = "    [" + issue.Author + "] " + issue.Message + " : " + compElelms[compElelms.Length - 1] + " : " + issue.Line + " : ";
                notes.AppendLine("    " + data + "[Open Issue | " + this.userConf.Hostname.TrimEnd('/') + "/issues/search#issues=" + issue.Key + "] [Open File |" + this.userConf.Hostname.TrimEnd('/') + "/component?id=" + issue.Component + "&line=" + issue.Line + "]");
                this.PopulateStatistics(issue);
            }
