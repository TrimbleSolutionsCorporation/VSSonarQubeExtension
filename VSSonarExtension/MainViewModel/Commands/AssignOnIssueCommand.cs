...
namespace VSSonarExtension.MainViewModel.Commands
{
    ...
    /// </summary>
    public class AssignOnIssueCommand : ICommand
    {
        /// <summary>
        ...
        public void Execute(object parameter)
        {
            ...
            if (list != null)
            {
                var status = this.service.AssignIssuesToUser(this.model.UserConfiguration, new System.Collections.Generic.List<Issue>(list.Cast<Issue>().ToList()), this.model.SelectedUser, this.model.CommentData);
                var allGood = true;
                foreach (var httpStatusCode in status)
                {
                    ...
                    {
                        allGood = false;
                        this.model.CustomPane.OutputString("Cannot Assign Issue: " + httpStatusCode.Key + " Return Code: " + httpStatusCode.Value);
